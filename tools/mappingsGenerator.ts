...
const mappersView = Object.entries(mappers).map(([key, value]) => `${key}:\n\t\t${value.join('\n\t\t')}`).join('\n\t')
console.log(`> loaded ${mappersLenght} mappers: \n\t${mappersView}\n`)

for (const [name, paths] of Object.entries(mappers)) {
  const mappings = fg.sync(paths).reduce((acc: Record<TokenKey, TokenMapping>, path: string) => ({ 
    ...acc,
    ...yaml.parse(fs.readFileSync(path, 'utf8'))
  }), {} as Record<TokenKey, TokenMapping>);

  fs.writeFileSync(
    `src/themes/presets/${name}/mappings.json`,
